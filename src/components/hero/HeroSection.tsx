...
export const HeroSection: React.FC<HeroSectionProps> = ({
  ...
  return (
    <section className="relative pt-6 pb-14 md:pt-14 md:pb-20 overflow-hidden">
        ...
        <div className="space-y-6 flex flex-col items-center">
          ...
          <motion.div variants={itemVariants} className="inline-block">
            <div className="inline-flex items-center gap-2.5 px-3.5 py-1.5 rounded-full glass-card border border-cyan-400/30 text-cyan-300 text-xs font-mono shadow-sm">
              ...
              <span className="text-slate-400 text-[11px]">{t.common.clientSideSecured}</span>
            </div>
          </motion.div>

          {/* Brand Headline */}
          <motion.div variants={itemVariants} className="space-y-2 max-w-3xl">
            <div className="text-xs sm:text-sm font-mono uppercase tracking-widest text-cyan-400 font-bold">
              {t.common.brandName}
            </div>
          ...
          <motion.div variants={itemVariants} className="w-full max-w-xl">
            ...
            <div className="mt-3 flex flex-wrap items-center justify-center gap-1.5 text-xs">
              <span className="text-slate-500 font-mono text-[11px] mr-1">{t.hero.quickTagsLabel}</span>
              {quickTags.map((tag) => (
                <Link
                  key={tag.slug}
                  to={`/tools/${tag.slug}`}
        ...
        >
          <div className="p-3.5 rounded-2xl glass-card border border-white/5 flex items-center gap-3">
            ...
            <div className="truncate text-left">
              <div className="font-bold text-xs text-slate-200 truncate">{t.stats.privacyRate}</div>
              <div className="text-[11px] text-slate-400 truncate">{t.stats.privacyLabel}</div>
            </div>
          </div>
          ...
          <div className="p-3.5 rounded-2xl glass-card border border-white/5 flex items-center gap-3">
            ...
            <div className="truncate text-left">
              <div className="font-bold text-xs text-slate-200 truncate">{t.stats.speedRate}</div>
              <div className="text-[11px] text-slate-400 truncate">{t.stats.speedLabel}</div>
            </div>
          </div>
          ...
          <div className="p-3.5 rounded-2xl glass-card border border-white/5 flex items-center gap-3">
            ...
            <div className="truncate text-left">
              <div className="font-bold text-xs text-slate-200 truncate">{t.stats.costRate}</div>
              <div className="text-[11px] text-slate-400 truncate">{t.stats.costLabel}</div>
            </div>
          </div>
          ...
          <div className="p-3.5 rounded-2xl glass-card border border-white/5 flex items-center gap-3">
            ...
            <div className="truncate text-left">
              <div className="font-bold text-xs text-slate-200 truncate">{t.stats.toolsCount}</div>
              <div className="text-[11px] text-slate-400 truncate">{t.stats.toolsLabel}</div>
            </div>
          </div>
        </motion.div>
      </motion.div>
