...
return (
<CardBase template={template} side="front" cardRef={cardRef}>
...
AHADEX
      ...
      <div>
        ...
        <p className="mt-1 text-xs" style={{ color: template.muted }}>
          {company}
        </p>
      </div>

      <div className="flex items-end justify-between gap-5">
        <div className="space-y-1 text-[8px]" style={{ color: template.muted }}>
...
/>
    ...
    <div className="relative flex h-full flex-col justify-center px-12">
      ...
      >
        {(name.match(/[A-Z]/g) || ['A', 'M']).slice(0, 2).join('')}
      </div>
...
/>
        ...
        <div>
          ...
          <p className="mt-1 text-xs font-bold">{company}</p>
        </div>
      </div>
...
/>
  ...
  <div className="min-w-0">
    ...
    <p
      ...
      style={{ color: template.muted }}
    >
      {data.bio || 'Your professional introduction appears here.'}
    </p>
  </div>
</div>
...
return (
<CardBase template={template} side="back" cardRef={cardRef}>
<div
...
/>
  ...
  <div className="relative flex h-full items-center justify-between gap-8 p-10">
    <div className="min-w-0">
      ...
      >
        <ContactItem icon={Phone} value={data.phone} template={template} />
        <ContactItem icon={MessageCircle} value={data.whatsapp} template={template} />
        <ContactItem icon={Mail} value={data.email} template={template} />
        <ContactItem icon={Globe} value={data.website} template={template} />
        <ContactItem icon={MapPin} value={data.address} template={template} />
        <ContactItem icon={Linkedin} value={data.linkedin} template={template} />
...
const { templateId, mode: modeParam } = useParams<{
templateId?: string;
mode?: string;
}>();
...
const update = (key: keyof CardData, value: string | null) => {
setData((current) => ({
...current,
[key]: value,
}));
...
if (!file.type.startsWith('image/')) {
  toast?.showToast?.('Please select an image file.');
  return;
}

if (file.size > 8 * 1024 * 1024) {
  toast?.showToast?.('Image must be 8MB or smaller.');
  return;
}
...
const buildCard = () => {
setBuilt(true);
toast?.showToast?.('Card preview updated.');
};
...
setSide('front');
setBuilt(false);
toast?.showToast?.('Your card fields were cleared.');
};

const getExportSource = (targetSide: 'front' | 'back') => {
...
if (!source) {
  throw new Error(`Unable to find ${targetSide} card.`);
}
...
try {
  const targets: Array<'front' | 'back'> =
    mode === 'two-side' ? ['front', 'back'] : ['front'];
  ...
  toast?.showToast?.(
    mode === 'two-side'
      ? 'Front and back JPG files downloaded.'
      : 'JPG downloaded at 1050 × 600 px.',
  );
} catch (error) {
  console.error(error);
  toast?.showToast?.('JPG export failed. Please try again.');
} finally {
  setBusy(false);
}
...
try {
  ...
  console.error(error);
  toast?.showToast?.('PDF export failed. Please try again.');
} finally {
  setBusy(false);
}
...
const currentCard = mode === 'two-side'
? side === 'front'
? (
<TwoSideFront
template={template}
      ...
      <Reveal>
        <section className="mb-7">
          <p
            ...
            style={{ color: template.accent }}
          >
            {mode === 'two-side' ? 'Two-Side Studio' : 'Single-Side Studio'}
          </p>
          ...
          <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-400 sm:text-base">
            Your selected template stays intact while your own information,
            photo and branding are added. Nothing from the demo preview is
            carried into your final card.
          </p>
        </section>
      </Reveal>
